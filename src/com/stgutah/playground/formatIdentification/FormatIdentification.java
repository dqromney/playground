...
public class FormatIdentification
{
	...
	private static void init()
	{
		...
		try
		{
            File inputFile = new File("/Users/dqromney/stgdev/playground/src/com/stgutah/playground/formatIdentification/formats.txt");
//            File inputFile = new File("formats.txt");
            String msg = MessageFormat.format("inputFile: {0}", inputFile == null ? "null" : inputFile.getCanonicalFile());
            System.out.println(msg);

			InputStream input = // descriptions.getClass().getResourceAsStream("/metadata/formats.txt");
            new FileInputStream(inputFile.getCanonicalFile());
            // FormatIdentification.class.getClassLoader().getResourceAsStream("/formats.txt");
			if (input == null)
			{
				return;
			}
